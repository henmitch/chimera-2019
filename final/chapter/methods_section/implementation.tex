...
We verified the code by reproducing the results of [\onlinecite{Santos2017}].
...
The chimeras were extremely unlikely to be eliminated on such a time scale, due to the size of the network [\onlinecite{Wolfrum2011}].
...
\end{equation}
where $t_{i}$ is the time at which the $j$th neuron fires ($x_{j}$ crosses 0 in a positive direction) for the $i$th time\footnote{This is a similar measure for the phase as was used in [\onlinecite{Santos2017}], but allows for easier discrimination between physical and aphysical parameter sets.
