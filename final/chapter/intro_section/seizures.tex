...
\textit{Neurons} are cells which are specialized for communication (see \cref{fig:neuron_diagram} for a diagram\todo{Cite}).
\begin{figure}[ht]
  \centering
...
And example of a neural mass model is the extremely simple Wilson-Cowan model \cite{Wang2012}:
\begin{align}
  \tau_{x} \dot{x}
  &=
...
An important matter to note is that $\tau_{0} \gg \tau_{2} \gg \tau_{1}$.
...
This means that it does not provide insight into the underpinnings and root causes behind seizures, simply describes symptoms.
